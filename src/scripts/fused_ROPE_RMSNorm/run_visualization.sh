...
# SCRIPT_DIR：脚本自身所在目录的绝对路径（即项目根目录 CUDA_ROPE/）
#
#   $0                    脚本的调用路径，如 ./run_visualization.sh
...
LOG_FILE="$PROJECT_ROOT/logs/visualize_rope_rotation.log"
...
# visualize 脚本现与本 sh 同在 src/scripts/fused_ROPE_RMSNorm/ 下，故用 $SCRIPT_DIR（脚本自身目录）。
uv run python "$SCRIPT_DIR/visualize_rope_rotation.py" 2>&1 | tee "$LOG_FILE"
