...
#pragma unroll
      for (uint i = 0; i < elementNumPerThread / 2; i++) {
        ...
        auto eleCos = _typeConvert<scalar_t_cache>::convert(ROPE_LDG(cos + offsetCosSinPerThread + i));
        auto eleSin = _typeConvert<scalar_t_cache>::convert(ROPE_LDG(sin + offsetCosSinPerThread + i));
